...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const insertLayer = useMutation((
    ...
    if(liveLayers.size >= CANVAS_MAX_LAYERS_TRASHHOLD) {
      return;
    }
  ...
  const startMultiSelection = useCallback((
    ...
    if(Math.abs(current.x - origin.x) + Math.abs(current.y - origin.y) > SELECTION_NET_TRASHHOLD) {
      setCanvasState({
        mode: CanvasMode.SelectionNet,
        origin,
        current,
  ...
  const insertPath = useMutation((
    ...
    if(
      ...
      liveLayers.size >= CANVAS_MAX_LAYERS_TRASHHOLD
    ) {
    ...
    <main className='w-full h-full relative bg-neutral-100 touch-none'>
      ...
      >
        ...
        >
          ...
          <SelectionBox 
            onResizeHandleOinterDown={onResizeHandlePointerDown}
          />
